...
const getPedidos = async (req, res, next) => {
  try {
    await PedidoModel.PedidoModelfind({})
      .then((succ) => {
...
const getPedidosByCompania = async (req, res, next) => {
  ...
  try {
    let data = [];
    await PedidoModel.PedidoModel.find({ idCompania: id })
      .populate(["idUsuario", "idCompania"])
      .then(async (_data) => {
        for (const item of _data) {
          const orderDetail = await PedidoModel.DetallePedidoModel.find({
            idPedido: item["_id"],
          }).populate(["idProducto"]);
...
const getPedidosByUser = async (req, res, next) => {
  ...
  try {
    let data = [];
    await PedidoModel.PedidoModel.find({ idUsuario: id })
      .populate(["idUsuario", "idCompania"])
      .then(async (_data) => {
        for (const item of _data) {
          const orderDetail = await PedidoModel.DetallePedidoModel.find({
...
const createPedido = async (req, res, next) => {
  const { idCompania, idUsuario, data, facturado } = req.body;
...
const updatePedido = async (req, res, next) => {
  let { id } = req.params;
  const { idProducto, idUsuario, cantidad, subtotal, total, confirmado } =
    req.body;
  try {
    const updatePedidoA = {
    ...
    await PedidoModel.PedidoModelfindByIdAndUpdate({ _id: id }, updatePedidoA)
      .then((success) => {
...
const deletePedido = async (req, res, next) => {
  try {
    const { _id } = req.body;
    await PedidoModel.PedidoModelfindOneAndDelete({ _id: id })
      .then(async (succ) => {
        await DetallePedidoModel.deleteMany({ idPedido: id });
        res
          .status(200)
          .json({ type: "ok", message: "Eliminado correctamente", data: succ });
      })
      .catch((err) =>
...
const confirmarPedido = async (req, res, next) => {
  ...
  try {
    await PedidoModel.DetallePedidoModel.findByIdAndUpdate(_id, {
      confirmado: confirmado,
    })
      .then((succ) => {
        res.status(200).json({
